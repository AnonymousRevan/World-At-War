...
NDefines.NProduction.BASE_FACTORY_SPEED_MIL = 3.60 				-- Base factory speed multiplier (how much hoi3 style IC each factory gives).
...
NDefines.NMilitary.ATTRITION_DAMAGE_ORG = 0.2					   -- damage from attrition to Organisation
...
NDefines.NAir.AIR_WING_BOMB_DAMAGE_FACTOR = 2.00					-- Used to balance the damage done while bombing.
...
NDefines.NAir.COMBAT_DAMAGE_SCALE_CARRIER = 10					-- same as above but used inside naval combat for carrier battle
...
NDefines.NNavy.ESCAPE_SPEED_HIDDEN_SUB = 0.50									-- hidden subs get faster escape speed
...
NDefines.NSupply.RAILWAY_BASE_FLOW = 13.0 		-- how much base flow railway gives when a node connected to its capital/a naval node by a railway
