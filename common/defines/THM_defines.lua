...
NDefines.NCountry.BASE_MAX_COMMAND_POWER = 150.0					-- base value for maximum command power, vanilla value is 80.0
NDefines.NCountry.ATTACHE_XP_SHARE = 0.15						-- Country received xp from attaches
NDefines.NCountry.SPECIAL_FORCES_CAP_BASE = 0.06					-- Max ammount of special forces battalions is total number of non-special forces battalions multiplied by this and modified by a country modifier
NDefines.NCountry.SPECIAL_FORCES_CAP_MIN = 45					-- You can have a minimum of this many special forces battalions, regardless of the number of non-special forces battalions you have, this can also be modified by a country modifier
...
NDefines.NProduction.BASE_COUNTRY_ENERGY_PRODUCTION = 55.0 			-- The base energy production of a country
...
NDefines.NProduction.BASE_FACTORY_SPEED_MIL = 3.4 				-- Base factory speed multiplier (how much hoi3 style IC each factory gives).
NDefines.NProduction.BASE_FACTORY_SPEED_NAV = 2.00				-- Base factory speed multiplier (how much hoi3 style IC each factory gives).
...
NDefines.NTechnology.BASE_YEAR_AHEAD_PENALTY_FACTOR = 3.75		-- Base year ahead penalty
...
NDefines.NTechnology.USE_BONUS_REGRET_TIMER = 30						-- Number of days the player has to regret using a limited tech bonus
...
NDefines.NMilitary.LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.058       -- global damage modifier... but some equipment is returned at end of battles see : EQUIPMENT_COMBAT_LOSS_FACTOR
...
NDefines.NMilitary.LAND_COMBAT_COLLATERAL_FORT_FACTOR = 0.007		-- Factor to scale collateral damage to forts with.
...
NDefines.NMilitary.ATTRITION_DAMAGE_ORG = 0.09					   -- damage from attrition to Organisation
NDefines.NMilitary.ATTRITION_EQUIPMENT_LOSS_CHANCE = 0.02		   -- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
...
NDefines.NMilitary.BASE_LEADER_TRAIT_GAIN_XP = 1.0			   -- Base xp gain for traits per hour for armies
...
NDefines.NMilitary.ENEMY_AIR_SUPERIORITY_SPEED_IMPACT = -0.20     -- effect on speed due to enemy air superiority
...
NDefines.NMilitary.FIELD_EXPERIENCE_MAX_PER_DAY = 4				-- Most xp you can gain per day
...
NDefines.NMilitary.OUT_OF_SUPPLY_ATTRITION = 0.50                 -- max attrition when out of supply
...
NDefines.NMilitary.STRATEGIC_SPEED_INFRA_MAX = 8.0              	-- Additional speed of strategic redeployment on max-level infrastructure
NDefines.NMilitary.STRATEGIC_SPEED_RAIL_BASE = 8.0              	-- Base speed of strategic redeployment when on railways
NDefines.NMilitary.STRATEGIC_SPEED_RAIL_MAX = 16.0                	-- Additional speed of strategic redeployment on max-level railways
...
NDefines.NMilitary.FIELD_MARSHAL_XP_RATIO = 0.9					-- xp gain ratio for army group leaders
...
NDefines.NAir.AIR_WING_MAX_STATS_SPEED = 850
NDefines.NAir.AIR_WING_MAX_STATS_BOMBING = 250
NDefines.NAir.AIR_WING_BOMB_DAMAGE_FACTOR = 2.25					-- Used to balance the damage done while bombing.
NDefines.NAir.BIGGEST_AGILITY_FACTOR_DIFF = 3.0				-- biggest factor difference in agility for doing damage (caps to this)
NDefines.NAir.BIGGEST_SPEED_FACTOR_DIFF = 2.45				-- biggest factor difference in speed for doing damage (caps to this)
NDefines.NAir.TOP_SPEED_DAMAGE_BONUS_FACTOR = 0.05			-- A factor for scaling the top speed of a plane into damage buff. If an attacking wing has a speed advantage of any form their speed value will be converted into a percentage bonus with this modifier
NDefines.NAir.COMBAT_DAMAGE_STATS_MULTILPIER = 0.4
NDefines.NAir.COMBAT_MULTIPLANE_CAP = 1.0						-- How many planes can shoot at each plane on other side ( if there are 100 planes we are atttacking COMBAT_MULTIPLANE_CAP * 100 of our planes can shoot )
NDefines.NAir.COMBAT_DAMAGE_SCALE = 0.35							-- Higher value = more shot down planes
NDefines.NAir.COMBAT_DAMAGE_SCALE_CARRIER = 8					-- same as above but used inside naval combat for carrier battle
NDefines.NAir.DETECT_CHANCE_FROM_RADARS = 0.7					-- How much the radars in area affects detection chance.
...
NDefines.NAir.ANTI_AIR_PLANE_DAMAGE_FACTOR = 0.60					-- Anti Air Gun Damage factor
NDefines.NAir.ANTI_AIR_PLANE_DAMAGE_CHANCE = 0.10					-- Anti Air Gun hit chance
...
NDefines.NAir.DISRUPTION_FACTOR_CARRIER = 5.0							-- multiplier on disruption damage to scale its effects on carrier vs carrier planes
NDefines.NAir.DISRUPTION_ATTACK_FACTOR = 0.75
NDefines.NAir.CARRIER_PLANES_AMOUNT_FOR_POSITIONING = 40         -- below this amount of planes on a carrier we no longer get max benefit on enemy positioning
...
NDefines.NNavy.COMBAT_TORPEDO_CRITICAL_DAMAGE_MULT = 1.0						-- multiplier to damage when got critical hit from torpedo. (Critical hits are devastating as usualy torpedo_attack are pretty high base values).
...
NDefines.NNavy.NAVAL_TRANSFER_BASE_SPEED = 10                                  -- base speed of units on water being transported
...
NDefines.NNavy.CAPITAL_ONLY_COMBAT_ACTIVATE_TIME = 24                          -- hours from start of combat when only carriers, capitals and subs get to attack
...
NDefines.NNavy.UNDERWAY_REPLENISHMENT_RANGE_FACTOR = 0.25			-- bonus factor applied to task force's range when underway replenishment is activated (e.g. 0.2 means +20%)
NDefines.NNavy.UNDERWAY_REPLENISHMENT_CONVOY_COST_PER_FUEL = 0.30	-- Cost in convoys for underway replenishment multiplied by max daily fuel consumption (rounded up)
...
NDefines.NNavy.CONVOY_HIT_PROFILE												= 100.0  	-- convoys has this contant hit profile
...
NDefines.NNavy.SUBMARINE_HIDE_TIMEOUT 											= 50		-- Amount of in-game-hours that takes the submarine (with position unrevealed), to hide.
NDefines.NNavy.SUBMARINE_REVEALED_TIMEOUT 										= 50		-- Amount of in-game-hours that makes the submarine visible if it is on the defender side.
...
NDefines.NRailwayGun.ATTACK_TO_ENTRENCHMENT_MODIFIER_FACTOR = 0.5		-- Entrenchment modifier is calculated by multiplying railway gun attack value with this and dividing by 100
NDefines.NRailwayGun.ATTACK_TO_BOMBARDMENT_MODIFIER_FACTOR = 0.175	-- Bombardment modifier is calculated by multiplying railway gun attack value with this and dividing by 100
...
NDefines.NRaids.RAID_MEDIUM_RISK_SETTING_DISASTER_MODIFIER = 0.10  -- How much the disaster risk is modified when the dial is set to "medium"
NDefines.NRaids.RAID_HIGH_RISK_SETTING_DISASTER_MODIFIER = 0.25   -- How much the disaster risk is modified when the dial is set to "high"

NDefines.NDoctrines.TRAINING_MASTERY_GAIN_FACTOR = 0.25
NDefines.NDoctrines.MAX_MONTHLY_MASTERY_GAIN = 75.0
NDefines.NDoctrines.MIN_MASTERY_GAIN_PER_DAY = 0.0

NDefines.NProject.BREAKTHROUGH_DAILY_TECHNOLOGY_GAIN = 40
NDefines.NProject.BASIC_RESEARCH_TECHNOLOGY_BONUS_FACTOR = 0.05
NDefines.NProject.BASIC_RESEARCH_TECHNOLOGY_BONUS_DIMINISHING_RETURN_FACTOR = 0.25

NDefines.NMilitary.ATTRITION_WHILE_MOVING_FACTOR = 0.1
NDefines.NMilitary.RELIABILITY_WEATHER = 1.0